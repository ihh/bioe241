...
\end{eqnarray*}
 \item If $P_n$ is an $n$-dimensional Gaussian $\forall n, \{ t_1 \ldots t_n \}$, then $x(t)$ is a {\em Gaussian process}. The covariance matrix is $\sigma_{ij} = \expect{x(t_i) x(t_j)}$
...
\begin{frame}{}
 ...
 \item Case \#1: fixed basis functions (parameters indep. of ${\bf w}$)
\[
